...
#ifdef GAMEBOY
if (M == NULL)
    M = (unsigned int16 *)calloc (MEMSIZE >> 1, sizeof (unsigned int16));
#else
...
cpu_identify()
{
    printf("\nCPU    : PDP-11 (J-11 18 MHz)\n");
    printf("FPU    : FP11-B\n");
    printf("Memory : %d KB (18-bit phys)\n", (INIMEMSIZE >> 10));
...
if ((mc != 0) && !get_yn ("Really truncate memory [N]?", FALSE))
	return SCPE_OK;
#ifdef GAMEBOY
nM = (unsigned int16 *)calloc (value >> 1, sizeof (unsigned int16));
#else
