...
#ifdef GAMEBOY
unsigned int16 rk_fill[RK_NUMWD] = { 0 };
#else
...
#ifdef GAMEBOY
		fxwrite (rk_fill, sizeof (int16), fillc, uptr -> fileref);
#else
...
#ifdef GAMEBOY
#define BOOT_LEN (sizeof (rk_boot_rom) / sizeof (int))
#else
#define BOOT_LEN (sizeof (boot_rom) / sizeof (int))
#endif

#ifdef GAMEBOY
const int32 rk_boot_rom[] = {
#else
...
#ifdef GAMEBOY
for (i = 0; i < BOOT_LEN; i++) M[(BOOT_START >> 1) + i] = rk_boot_rom[i];
#else
...
rk_identify()
{
    printf("RK: RK11 cartridge disk controller\n");
    return SCPE_OK;
}
#endif
